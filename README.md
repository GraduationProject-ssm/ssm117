...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=113)
...
## 2.4 SSM框架
本网络教学平台基于Spring、SpringMVC、Mybatis框架进行开发设计。
...
### 3.1.1技术可行性
本网络教学平台的设计与实现采用Java技术和MYSQL数据库进行开发设计，作为计算机专业学生，在学校期间就接触到许多关于编程方面的知识，当然也包括各种编程软件，对他们的了解度也比较系统，所以技术开发上面还是有一定把握。
### 3.1.2操作可行性
...
### 3.1.4法律可行性
本网络教学平台的设计与实现开发的所有技术资料都为合法，知识产权问题不会发生在开发过程中，而且没有抄袭其他相关系统，不会有侵犯版权的问题。所以在开发过程中不会涉及法律责任。
...
## 3.3系统功能分析
此网络教学平台的设计与实现功能分析主要分为管理员功能模块、教师功能模块和学生功能模块三大模块，下面详细介绍这三大模块的主要功能：

（1）管理员：管理员登陆后可对系统进行全面管理，管理员主要功能模块包括个人中心、学生管理、教师管理、课堂信息管理、签到信息管理、教学资源管理以及留言板管理，管理员实现了对系统信息的查看、添加、修改和删除的功能。管理员用例图如图3-1所示。

![](/md/blog.003.png)

图3-1 管理员用例图

（2）学生：学生进入本网络教学平台的设计与实现前台可查看系统信息，包括首页、课堂信息、签到信息、教学资源、试卷列表以及留言反馈等，注册登录后主要功能模块包括个人中心、课堂信息管理、课堂加入管理、签到信息管理、签到情况管理、教学资源管理、留言板管理、我的收藏管理以及考试管理。

![](/md/blog.004.png)

图3-2 学生用例图

（3）教师：教师学生登录后主要实现的功能模块包括个人中心、课堂信息管理、课堂加入管理、签到信息管理、签到情况管理、教学资源管理、试题管理、留言板管理、试卷管理、考试管理。
...
# 4 系统设计
## 4.1系统概要设计
网络教学平台的设计与实现主要分为管理员、教师和学生三个角色，系统采用B/S结构(Browser/Server,浏览器/服务器结构)和基于Web服务两种模式，是一个适用于Internet环境下的模型结构。只要用户能连上Internet，便可以在任何时间、任何地点使用。系统工作原理图如图4-1所示：
...
在为实体设计属性与关联的时候便可以确定实体间主外键的关联关系，实体所具有的延伸性。在E-R图中这些都能直观的显示出来。像这样将数据分类成组后，就会产生了与相关的分组所不同的信心，这样更加的易于用户进行观察和实际分析。本网络教学平台的设计与实现的E-R图如下所示：
...
图4-10 学生实体属性E-R图
### 4.4.2 数据库表设计
本网络教学平台的设计与实现采用了MYSQL数据库管理系统，各个表的设计结果如下：
...
|shenfenzhenghaoma|varchar(200)|NULL|身份证号码|
|shoujihaoma|varchar(200)|NULL|手机号码|
|xiangpian|varchar(200)|NULL|相片|
